...
# Deterministic policy means output the actual action instead of a probability
# Will need a way to bound the actions to the envirnment limits
...
# The target actor is just the evaluator actor plus some noise process
# They used Ornstein Uhlenbeck -> Will need a class for noise
...
class OUActionNoise(object):
    def __init__(self, mu, sigma = 0.15, theta = 0.2, dt = 0.2, x0 = None):
        self.theta = theta
        self.mu = mu
        self.dt = dt
        self.sigma = sigma
...
class ReplayBuffer(object):
    ...
    def sample_buffer(self, buffer_size):
        ...
        return states, new_sates, actions, rewards, terminal

class Actor(object):
    def __int__(self, lr, n_actions, name, input_dins, sess, fc1_dims,
                fc2_dims, action_bound, bacth_size = 64, chkpt_dir = 'tmp/ddpg'):
        self.lr = lr
        self.n_actions = n_actions
        self.input_dins = input_dins
        self.name = name
        self.fc1_dins = fc1_dims
        self.fc2_dins = fc2_dims
        self.sess = sess
        self.batch_size = bacth_size
        self.action_bound = action_bound
        self.chpt_dir = chkpt_dir
        self.build_network()
        self.prams = tf.trainable_variables(scope = self.name)
        self.saver = tf.train.Saver()
        self.checkpoint_file = os.path.join(chkpt_dir, name + '_ddpg_ckpt')

        self.unnormalized_actor_gradients = tf.gradients(self.mu, self.params, 
                                                         -self.action_gradient)
        self.actor_gadients = list((map(lambda x: tf.div(x, self.batch_size),
                                           self.unnormalized_actor_gradients)))
        self.optimize = tf.train.AdamOptimizer(self.lr). \
            apply_gradients(zip(self.actor_gadients, self.params))
    ...
    def build_network(self):
        with tf.variable_scope(self.name):
            ...
            f1 = 1 / np.sqrt(self.fc1_dins)
            dense1 = tf.layers.dense(self.input, units = self.fc1_dins,
                                     kernel_inicialization = random_uniform(-f1, f1),
                                     bias_initializer = random_uniform(-f1, f1))
            batch1 = tf.layers.batch_normalization(dense1)
            layer1_activation = tf.nn.relu(batch1)

            f2 = 1 / np.sqrt(self.fc2_dins)
            dense2 = tf.layers.dense(layer1_activation, units = self.fc2_dins,
            ...
            f3 = 0.003
            mu = tf.layer.dense(layer2_activation, units = self.n_actions,
    ...
    def load_checkpoint(self):
        ...
        self.saver.restore(self.sess, self.checkpoit_file)
...
class Critic(object):
    ...
    def build_network(self):
        with tf.variable_scope(self.name):
            ...
            self.loss = tf.losses.mean_squared_error(self.q.target, self.q)
...
class Agent(object):
    def __init__(self, alpha, beta, input_dims, tau, env, gama = 0.99,
        ...
        self.updates_network_parameters(first = True)
    ...
    def learn(self):
        ...
        for j in range(self.batch_size):
            target.appand(reward[j] + self.gamma * critic_value[j] * done[j])
